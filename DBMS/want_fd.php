...
if($_SERVER["REQUEST_METHOD"]=="POST")
{
	 ...
	 if(!preg_match('/^[a-zA-Z]*$/',$_POST["withdraw"]))
		$withdrawErr="withdarw has to be a character!";
	else
	{
		$withdrawErr="";
...
<?php
if($trans_flag==1)
	echo "<br>$transErr,$withdrawErr,$idErr,$_POST[withdraw],$_POST[trans],$trans_flag,$_POST[id]";

if($transErr==""&& $withdrawErr=="" &&$idErr==""&&$_POST["withdraw"]!=""&&$_POST["trans"]!=""&&$trans_flag==0&&$_POST["id"]!="")
	{
		...
		else
		{
			...
			$interest=($principal*pow((1+($rate/$n)),($n*$t)))-$principal;
